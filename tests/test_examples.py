...
def test_event_picker_add_event(event_picker, setup_folders):
    ...
    current_id = picker.events["pick2"].data_id_func() # ID might change if signal changes
    ...
    assert len(picker.events["pick2"]._data[current_id].added) == 1 # Still 1 because secind click is before first click

    picker(simulate_key_press_at_xy(fax, key='2', xdata=0.1))
    picker(simulate_key_press_at_xy(fax, key='2', xdata=0.15))
    assert len(picker.events["pick2"]._data[current_id].added) == 2 # Still 1 because secind click is before first click
    ...
    assert len(picker.events["pick3"]._data[current_id].added) == 1 # Still 1 due to overwrite
...
def test_event_picker_remove_event(event_picker):
    ...
    remove_event = simulate_key_press_at_xy(fax, key='alt+1', xdata=0.605) # Close to 0.6
...
def test_selector_demo_start_stop(selector_demo):
    ...
    with patch('matplotlib.widgets.LassoSelector.__init__', return_value=None) as mock_lasso_init:
         # Need to mock the LassoSelector init as it requires interaction
        with patch.object(examples.LassoSelectorWidget, '__init__', return_value=None) as mock_lasso_widget_init:
