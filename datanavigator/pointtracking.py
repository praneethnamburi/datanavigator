...
class VideoPointAnnotator(VideoBrowser):
    ...
    def update_frame_marker(self, draw=False):
        ...
        self._plot_frames_of_interest_x.set_data(*utils.ticks_from_times(self.frames_of_interest, xl))
        self._plot_frames_of_interest_y.set_data(*utils.ticks_from_times(self.frames_of_interest, yl))
    ...
    def check_labels_with_lk(self, mode="minimal"):
        ...
        for label in label_list:
            frames = utils.List(sorted(list(source_ann[label].keys())))
            if mode == "minimal":
                c = self._current_idx
...
class VideoAnnotation:
    ...
    def __init__(self, fname: str=None, vname: str=None, name: str=None, **kwargs):
        ...
        self.palette = utils.get_palette(kwargs.pop('palette_name', 'Set2'), n_colors=10) # seaborn Set 2
        self.plot_handles = {
    ...
    def get_area(self, labels: list | str, lowpass: float=None) -> pysampled.Data | np.ndarray:
